    ...
    <div class="login_box">
      ...
      <el-form ref="loginFromRef" class="login_form" :model="loginForm" :rules="loginFormRules">
        ...
        <el-form-item prop="chatNum">
          <el-input v-model="loginForm.chatNum" prefix-icon="iconfont icon-huabanfuben"></el-input>
        </el-form-item>
        <!-- 密码 -->
  ...
  methods: {
    login() {
      this.$refs.loginFromRef.validate(async valid => {
        ...
        const { data: res } = await this.$http.post('/mychat/backgroup/adminlogin', JSON.stringify(this.loginForm))
        if (res.success === false) return this.$message.error('用户名或密码错误！');
